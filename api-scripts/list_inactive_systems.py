#!/usr/bin/env python3
"""
SUSE Manager / Uyuni API - List Inactive Systems
"""
...
def main():
    ...
    p.add_argument('-p', '--password'); p.add_argument('--days', type=int, default=30)
    ...
    try:
        ...
        print(f"[*] Listing systems inactive for > {args.days} days")
        for s in c.system.listInactiveSystems(k, args.days):
             print(f"{s['id']} | {s['name']} | {s['last_checkin']}")
        c.auth.logout(k)
