#!/usr/bin/env python3
"""
SUSE Manager / Uyuni API - List Systems Needing Reboot
"""
...
def main():
    ...
    try:
        ...
        systems = []
        try:
            systems = c.system.listSystemsWithRebootRequired(k)
        except:
            # Fallback
            print("[*] Fallback: iterating all systems...")
            for s in c.system.listSystems(k):
                try:
                    d = c.system.getDetails(k, s['id'])
                    if d.get('reboot_required'): systems.append(s)
                except: pass
        
        for s in systems: print(f"{s['id']} | {s['name']}")
