#!/usr/bin/env python3
"""
SUSE Manager / Uyuni API - Apply Salt State
"""
...
def main():
    ...
    p.add_argument('--sls', required=True); p.add_argument('--test', action='store_true')
    ...
    try:
        ...
        aid = c.system.scheduleApplySaltStates(k, args.sid, [args.sls], datetime.datetime.now(), args.test)
        print(f"[+] State '{args.sls}' Scheduled. Action ID: {aid}")
        c.auth.logout(k)
